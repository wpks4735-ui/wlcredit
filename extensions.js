...
window.addEventListener('DOMContentLoaded',()=>{q('#refreshBackups')?.addEventListener('click',loadBackups);q('#backupDateFilter')?.addEventListener('change',loadBackups);q('#createDailyBackup')?.addEventListener('click',saveBackup);document.addEventListener('click',e=>{if(e.target.closest('[data-section="dashboard"]'))setTimeout(loadBackups,50)});window.addEventListener('swk-language-applied',()=>{setLabels();loadBackups()});setLabels()});window.loadBackupCenter=loadBackups;document.addEventListener('wl:data-loaded',()=>{if(state.staff)loadBackups()});
})();
...
(() => {
 ...
 const videoExt=v=>/\.(mp4|mov|webm)(\?|$)/i.test(String(v||''));
 ...
 async function hydrateDocuments(){
   ...
   for(const el of host.querySelectorAll('[data-bucket][data-path]')){try{const url=await docUrl(el.dataset.bucket,el.dataset.path);el.innerHTML=imgExt(el.dataset.path)?`<a href="${esc(url)}" target="_blank"><img src="${esc(url)}" alt="${esc(el.dataset.label)}"></a>`:videoExt(el.dataset.path)?`<video controls preload="metadata" style="width:100%;max-height:300px" src="${esc(url)}"></video><a class="document-file-link" href="${esc(url)}" target="_blank">🎬 ${tx('Open video','打开影片','Buka video')}</a>`:`<a class="document-file-link" href="${esc(url)}" target="_blank">📄 ${tx('Open file','打开文件','Buka fail')}</a>`}catch(e){el.textContent=tx('Unable to load file','无法载入文件','Fail tidak dapat dimuatkan')}}
 }
 window.uploadCustomerDocument=async customerId=>{
   ...
   const isVideo=String(file.type||'').startsWith('video/');const maxSize=isVideo?50*1024*1024:10*1024*1024;if(file.size>maxSize)return toast(isVideo?tx('Maximum video size is 50 MB','影片最大为 50 MB','Saiz video maksimum ialah 50 MB'):tx('Maximum file size is 10 MB','文件最大为 10 MB','Saiz maksimum ialah 10 MB'),true);
 ...
 window.openCustomerProfile=id=>{
   ...
   const ec1={name:c.emergency_name||a?.emergency_name||'',relation:c.emergency_relation||a?.emergency_relation||'',phone:c.emergency_phone||a?.emergency_phone||''},ec2={name:c.emergency_name_2||a?.emergency_name_2||'',relation:c.emergency_relation_2||a?.emergency_relation_2||'',phone:c.emergency_phone_2||a?.emergency_phone_2||''};
   const personalBank=window.WLCustomerBank.read(c);
   const employment=[c.occupation,c.employer,c.monthly_salary!=null?c.monthly_salary:''].filter(v=>v!==''&&v!=null).join(' | ')||c.work_salary||'-';
...
(()=>{
 ...
 function render(){const body=$('#v51StaffRows')||$('#v311StaffProfitRows'),card=body?.closest('.v51-panel')||$('#v311StaffProfitCard');if(!card||!body||!st().staff)return;
 let filter=card.querySelector('#staffReportMonth');if(!filter){$('#staffReportMonth')?.remove();}if(!filter){filter=document.createElement('input');filter.type='month';filter.id='staffReportMonth';filter.setAttribute('aria-label','业绩月份');filter.value=month;(card.querySelector('.v51-report-head')||card.querySelector('.section-head')).append(filter);filter.onchange=()=>{if(/^\d{4}-\d{2}$/.test(filter.value)){month=filter.value;render()}}}
 const oldFilter=$('#v51StaffFilter');if(oldFilter)oldFilter.hidden=true;const exports=card.querySelector('.v51-downloads');if(exports)exports.hidden=true;
 ...
 const help=card.querySelector('.v51-report-head small')||$('#v311StaffProfitHelp');if(help)help.textContent=`统计月份：${month}；客户数及进行中贷款为当前数量。`;
 }
