(()=>{
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#loanApplicationForm')?.addEventListener('submit',async e=>{
   ...
   try{
    const docs={ic_front:await upload($('#applyIcFront').files[0],'ic-front'),ic_back:await upload($('#applyIcBack').files[0],'ic-back'),holding_ic:await upload($('#applyHoldingIc').files[0],'holding-ic'),payslip:await upload($('#applyPayslip').files[0],'payslip'),bank_statement:await upload($('#applyBankStatement').files[0],'bank-statement'),epf:await upload($('#applyEpfStatement').files[0],'epf-statement')};
    ...
    const extra=await appSb.rpc('public_save_application_extra',{p_application_code:x.data.application_code,p_employer_phone:$('#applyEmployerPhone').value,p_employer_pic_name:$('#applyEmployerPicName').value,p_employer_pic_phone:$('#applyEmployerPicPhone').value,p_document_paths:docs});
