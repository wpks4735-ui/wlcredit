...
-- V18 verification and compatibility checks.
-- Public customer identity must always be WL001, WL002, ...
update public.customers
set username = customer_code
where username is distinct from customer_code;

-- Keep the sequence beyond all current public usernames.
select setval(
  'public.customer_username_seq',
  greatest(
    coalesce((
      select max((substring(username from '[0-9]+$'))::bigint)
      from public.customers
      where username ~ '^WL[0-9]+$'
    ), 0),
    1
  ),
  true
...
-- Verification query (safe to run separately):
-- select username, customer_code, full_name, phone, id_number
-- from public.customers
-- order by substring(username from '[0-9]+$')::int;
