...
(async()=>{
 ...
 expect(!api.approved({status:'pending_disbursement'}),'pending gated');expect(!api.approved({status:'rejected'}),'rejected gated');expect(api.approved(l),'active allowed');
 ...
 const docs=[{id:'d1',storage_path:'customer-1/a.pdf',file_name:'Example-statement.pdf',mime_type:'application/pdf'},{id:'d2',storage_path:'customer-1/photo.png',file_name:'Example-photo.png',mime_type:'image/png'},{id:'d3',storage_path:'customer-1/clip.webm',file_name:'Example-video.webm',mime_type:'video/webm'}];
 window.sb={storage:{from:()=>({download:async p=>({data:p.endsWith('.pdf')?pdfBlob:image}),createSignedUrl:async()=>({data:{signedUrl:videoUrl}})})}};
 const bytes=await api.build(c,l,docs);const output=await PDFLib.PDFDocument.load(bytes);expect(output.getPageCount()>=8,'all PDF/image/video pages appended');
 ...
 return {bytes:Array.from(bytes),pages:output.getPageCount(),checks:12};
 });
