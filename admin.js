...
// Canonical customer login identity. `customer_code` remains a database compatibility alias.
function normalizeCustomerUsername(value){
 const raw=String(value||'').trim().toUpperCase();
 const match=raw.match(/(\d+)$/);
 if(!match)return raw;
 // Canonical public identity: WL001, WL002, ...
 if(/^WL\d+$/.test(raw))return 'WL'+String(Number(match[1])).padStart(3,'0');
 // Legacy identities such as SWKC000018 are display/login aliases only.
 if(/^(?:SWKC|CUS|CUSTOMER|C)\d+$/.test(raw))return 'WL'+String(Number(match[1])).padStart(3,'0');
 return raw;
}
...
function canonicalLoanId(value){
 const raw=String(value||'').trim().toUpperCase();
 const match=raw.match(/(\d+)$/);
 return match?'L'+String(Number(match[1])).padStart(5,'0'):raw;
}
window.normalizeCustomerUsername=normalizeCustomerUsername;
window.customerUsername=customerUsername;
window.canonicalLoanId=canonicalLoanId;
...
function applyV7Labels(){const l=SWK_LANG.current||'en',m={en:{audit:'Audit Log',today:'Today',yesterday:'Yesterday',thisWeek:'This Week',lastWeek:'Last Week',thisMonth:'This Month',lastMonth:'Last Month',apply:'Apply',to:'to',search:'Search Username / Loan ID / Customer / IC / Phone / Payment ID',auditSearch:'Search staff, action or ID'},zh:{audit:'操作记录',today:'今天',yesterday:'昨天',thisWeek:'本周',lastWeek:'上周',thisMonth:'本月',lastMonth:'上个月',apply:'查询',to:'至',search:'搜索用户名 / 贷款编号 / 客户 / IC / 电话 / 付款编号',auditSearch:'搜索员工、操作或编号'},ms:{audit:'Log Aktiviti',today:'Hari Ini',yesterday:'Semalam',thisWeek:'Minggu Ini',lastWeek:'Minggu Lepas',thisMonth:'Bulan Ini',lastMonth:'Bulan Lepas',apply:'Cari',to:'hingga',search:'Cari nama pengguna / ID Pinjaman / Pelanggan / IC / Telefon / ID Bayaran',auditSearch:'Cari staf, tindakan atau ID'}}[l]||{};const nav=$('#nav button[data-section="auditLogs"]');if(nav)nav.textContent=m.audit;const keys=['today','yesterday','thisWeek','lastWeek','thisMonth','lastMonth'];$$('.date-preset').forEach((b,i)=>b.textContent=m[keys[i]]);if($('#applyDateRange'))$('#applyDateRange').textContent=m.apply;if($('#auditApply'))$('#auditApply').textContent=m.apply;if($('#dateRangeSeparator'))$('#dateRangeSeparator').textContent=m.to;if($('#globalSearch'))$('#globalSearch').placeholder=m.search;if($('#auditSearch'))$('#auditSearch').placeholder=m.auditSearch}
...
function renderGlobalSearch(){
 ...
 const customerMatches=state.customers.filter(c=>[customerUsername(c),c.full_name,c.id_number,c.phone].join(' ').toLowerCase().includes(q)).slice(0,6);
 const loanMatches=state.loans.filter(l=>[l.loan_id,canonicalLoanId(l.loan_id),l.customers?.full_name,customerUsername(l.customers)].join(' ').toLowerCase().includes(q)).slice(0,6);
 const paymentMatches=state.submissions.filter(x=>[x.id,x.loans?.loan_id,canonicalLoanId(x.loans?.loan_id),x.customers?.full_name,customerUsername(x.customers)].join(' ').toLowerCase().includes(q)).slice(0,4);
 const items=[...customerMatches.map(c=>`<button type="button" onclick="document.querySelector('#globalSearchResults').classList.add('hidden');openCustomerProfile('${c.id}')"><strong>${esc(customerUsername(c))} · ${esc(c.full_name)}</strong><small>${esc(c.id_number||'')} · ${esc(c.phone||'')}</small></button>`),...loanMatches.map(l=>`<button type="button" onclick="document.querySelector('#globalSearchResults').classList.add('hidden');openCustomerProfile('${l.customer_id}')"><strong>${esc(canonicalLoanId(l.loan_id))} · ${esc(l.customers?.full_name||'')}</strong><small>${money(l.principal)} · ${esc(loanStatus(l))}</small></button>`),...paymentMatches.map(x=>`<button type="button" onclick="document.querySelector('#globalSearchResults').classList.add('hidden');openPendingPayments()"><strong>${esc(x.loans?.loan_id?canonicalLoanId(x.loans.loan_id):x.id)}</strong><small>${money(x.amount)} · ${esc(x.status)}</small></button>`)].slice(0,12);
 box.innerHTML=items.join('')||`<div class="global-search-empty">${esc(v10t('noRecords'))}</div>`;box.classList.remove('hidden');
}

function renderCustomers(){
 const q=($('#customerSearch').value||'').trim().toLowerCase();
 const activeLabel=SWK_LANG.current==='zh'?'启用':SWK_LANG.current==='ms'?'Aktif':'Active';
 const inactiveLabel=SWK_LANG.current==='zh'?'停用':SWK_LANG.current==='ms'?'Tidak Aktif':'Inactive';
 const viewLabel=SWK_LANG.current==='zh'?'查看':SWK_LANG.current==='ms'?'Lihat':'View';
 const passwordLabel=SWK_LANG.current==='zh'?'修改密码':SWK_LANG.current==='ms'?'Tukar Kata Laluan':'Change Password';
 $('#customerRows').innerHTML=state.customers
  .filter(c=>[customerUsername(c),c.full_name,c.phone,c.id_number].join(' ').toLowerCase().includes(q))
  .map(c=>{
...
window.toggleCustomerStatus=async(id,enable)=>{
 ...
 toast(SWK_LANG.current==='zh'?(enable?'客户已启用':'客户已停用'):SWK_LANG.current==='ms'?(enable?'Pelanggan diaktifkan':'Pelanggan dinyahaktifkan'):(enable?'Customer enabled':'Customer disabled'));
 await loadAll();
};
function renderLoans(){
 $('#loanRows').innerHTML=state.loans.map(l=>{const c=state.customers.find(x=>x.id===l.customer_id),contacts=[c?.telegram_contact?.label,c?.whatsapp_contact?.label].filter(Boolean).join(' + ');return `<tr><td>${esc(canonicalLoanId(l.loan_id))}</td><td><span class="click-link" onclick="openCustomerProfile('${l.customer_id}')">${esc(l.customers?.full_name)}</span></td><td>${money(l.principal)}</td><td>${money(l.interest)}</td><td>${money(l.settlement_amount)}</td><td>${esc(c?.receiving_bank?.bank_name||tr('unassigned'))}</td><td>${esc(contacts||tr('unassigned'))}</td><td>${date(l.due_date)}</td><td><span class="badge ${l.status==='paid'?'ok':l.due_date<isoToday()?'danger':'warn'}">${loanStatus(l)}</span></td><td><button class="btn btn-secondary" onclick="openLoan('${l.id}')">${esc(v11t('edit'))}</button></td></tr>`}).join('')
}
function renderRepayments(){$('#repaymentRows').innerHTML=state.repayments.map(r=>`<tr><td>${date(r.payment_date)}</td><td>${esc(canonicalLoanId(r.loans?.loan_id))}</td><td>${esc(r.loans?.customers?.full_name)}</td><td>${money(r.amount)}</td><td>${esc(r.staff_profiles?.full_name||'-')}</td><td>${esc(r.notes)}</td></tr>`).join('')}
...
function renderSubmissions(){const rows=state.filter==='all'?state.submissions:state.submissions.filter(x=>x.status===state.filter);$('#submissionRows').innerHTML=rows.map(x=>`<tr><td>${date(x.created_at)}</td><td>${esc(x.customers?.full_name)}</td><td>${esc(canonicalLoanId(x.loans?.loan_id))}</td><td>${money(x.amount)}</td><td><button class="btn btn-secondary" onclick="viewReceipt('${x.id}')">View</button></td><td><span class="badge ${x.status==='approved'?'ok':x.status==='rejected'?'danger':'warn'}">${x.status}</span></td><td>${x.status==='pending'?`<button class="btn btn-primary" onclick="approveSubmission('${x.id}')">Approve</button> <button class="btn btn-danger" onclick="rejectSubmission('${x.id}')">Reject</button>`:''}</td></tr>`).join('')}
...
window.openLoan=(id,customerId)=>{
 ...
 const renderCustomerOptions=()=>{
   ...
   const keep=String(select?.value||selectedCustomerId||'');
   select.innerHTML=list.map(c=>`<option value="${esc(c.id)}" ${keep===String(c.id)?'selected':''}>${esc(customerUsername(c))} — ${esc(c.full_name||'-')} — ${esc(c.phone||'')}</option>`).join('');
   if(!select.value&&list.length) select.value=String(list[0].id);
   if(hint) hint.textContent=list.length?`${list.length} ${SWK_LANG.current==='zh'?'位客户':SWK_LANG.current==='ms'?'pelanggan':'customers'}`:(SWK_LANG.current==='zh'?'没有找到客户':SWK_LANG.current==='ms'?'Pelanggan tidak ditemui':'No customer found');
 };
 search?.addEventListener('input',renderCustomerOptions);
 ...
 form.onsubmit=async e=>{
   ...
   let x;
   if(id)x=await sb.from('loans').update({customer_id:customer,principal:p,interest:i,settlement_amount:s,remaining_amount:l.status==='paid'?0:s,disbursement_date:f.get('disb'),due_date:f.get('due'),expected_payment_at:f.get('expected_at')?new Date(f.get('expected_at')).toISOString():null,notes:f.get('notes'),updated_at:new Date().toISOString()}).eq('id',id);
   else x=await sb.rpc('staff_create_loan_auto',{p_customer_id:customer,p_principal:p,p_interest:i,p_settlement_amount:s,p_disbursement_date:f.get('disb'),p_due_date:f.get('due'),p_notes:f.get('notes')});
   if(x.error)return toast(x.error.message,true);
   closeModal();toast(tr('saved'));loadAll();
 };
}


window.manageCustomerContact=id=>{if(!requirePerm('contacts_assign'))return;const contact=state.contacts.find(x=>x.id===id);if(!contact||!canManageContact(contact))return toast(tr('noAccess'),true);const field=contact.channel_type==='telegram'?'assigned_telegram_id':contact.channel_type==='whatsapp'?'assigned_whatsapp_id':null;if(!field)return toast(SWK_LANG.current==='zh'?'只有 WhatsApp 和 Telegram 可以分配给客户。':SWK_LANG.current==='ms'?'Hanya WhatsApp dan Telegram boleh ditugaskan kepada pelanggan.':'Only WhatsApp and Telegram can be assigned to customers.',true);const customers=isSuperAdmin()?state.customers:state.customers.filter(isMine);modal(`<h2>${tr('manageAssignment')} · ${esc(contact.label)}</h2><form id="customerContactForm"><div class="checkbox-list">${customers.map(c=>`<label class="checkbox-row"><input type="checkbox" value="${c.id}" ${c[field]===id?'checked':''}><span>${esc(customerUsername(c))} — ${esc(c.full_name)}</span></label>`).join('')}</div><p><button class="btn btn-primary">${tr('save')}</button></p></form>`);$('#customerContactForm').onsubmit=async e=>{e.preventDefault();const selected=$$('#customerContactForm input:checked').map(x=>x.value),x=await sb.rpc('staff_assign_customers_contact',{p_customer_ids:selected,p_contact_id:id});if(x.error)return toast(x.error.message,true);closeModal();toast(tr('assignedCount',{count:x.data}));loadAll()}}
...
renderCustomers=function(){
  ...
  const filtered=(state.customers||[]).filter(c=>{
    const match=[customerUsername(c),c.full_name,c.phone,c.id_number].join(' ').toLowerCase().includes(q);
...
window.toggleCustomerStatus=async(id,enabled)=>{
  ...
  const question=SWK_LANG.current==='zh'?`确定要${verb}用户 ${customerUsername(c)} 吗？`:SWK_LANG.current==='ms'?`Sahkan untuk ${verb} pengguna ${customerUsername(c)}?`:`Are you sure you want to ${verb} user ${customerUsername(c)}?`;
...
renderSubmissions=function(){
 ...
 $('#submissionRows').innerHTML=rows.map(x=>`<tr><td>${date(x.created_at)}</td><td>${esc(x.customers?.full_name)}</td><td>${esc(canonicalLoanId(x.loans?.loan_id))}</td><td>${money(x.amount)}</td><td><button class="btn btn-secondary" onclick="viewReceipt('${x.id}')">${SWK_LANG.current==='zh'?'查看':'View'}</button></td><td><span class="badge ${x.status==='approved'?'ok':x.status==='rejected'?'danger':'warn'}">${SWK_LANG.current==='zh'?({pending:'待审核',approved:'已批准',rejected:'已拒绝'}[x.status]||x.status):x.status}</span></td><td>${x.status==='pending'?`<button class="btn btn-primary" onclick="approveSubmission('${x.id}')">${SWK_LANG.current==='zh'?'批准':'Approve'}</button> <button class="btn btn-danger" onclick="rejectSubmission('${x.id}')">${SWK_LANG.current==='zh'?'拒绝':'Reject'}</button>`:''}</td></tr>`).join('')||`<tr><td colspan="7" class="muted">${tr('noRecords')}</td></tr>`;
};
