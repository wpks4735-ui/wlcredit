...
window.openCustomer=function(id){
 ...
 <h3>${L('个人银行资料','Personal Bank Information','Maklumat Bank Peribadi')}</h3><div class="grid2"><div class="field"><label>${L('银行名称','Bank Name','Nama Bank')}</label><input name="bank_name" value="${E(personalBank.bank_name||'')}"></div><div class="field"><label>${L('户名','Account Name','Nama Akaun')}</label><input name="bank_account_name" value="${E(personalBank.bank_account_name||'')}"></div><div class="field"><label>${L('银行账号','Account Number','Nombor Akaun')}</label><input name="bank_account_number" value="${E(personalBank.bank_account_number||'')}"></div></div>
 ...
 let savedCustomerId=id, saving=false;
 const uploaded=new Set();
 const form=$('#customerFormV252');form.onsubmit=async e=>{e.preventDefault();if(saving)return;saving=true;const submit=form.querySelector('button');submit.disabled=true;try{const f=new FormData(form),data={full_name:f.get('name'),phone:f.get('phone'),id_number:f.get('idn'),address:f.get('address'),occupation:f.get('occupation')||null,employer:f.get('employer')||null,monthly_salary:Number(f.get('monthly_salary')||0)||null,salary_frequency:f.get('salary_frequency')||null,employer_phone:f.get('employer_phone')||null,employer_pic_name:f.get('employer_pic_name')||null,employer_pic_phone:f.get('employer_pic_phone')||null,bank_name:f.get('bank_name')||null,bank_account_name:f.get('bank_account_name')||null,bank_account_number:f.get('bank_account_number')||null,emergency_name:f.get('emergency_name')||null,emergency_relation:f.get('emergency_relation')||null,emergency_phone:f.get('emergency_phone')||null,emergency_name_2:f.get('emergency_name_2')||null,emergency_relation_2:f.get('emergency_relation_2')||null,emergency_phone_2:f.get('emergency_phone_2')||null,internal_notes:f.get('notes'),is_active:f.get('active')==='on',assigned_bank_id:f.get('bank_id')||null,assigned_whatsapp_id:f.get('whatsapp_id')||null,assigned_telegram_id:f.get('telegram_id')||null,updated_at:new Date().toISOString()};

 for(const k of ['bank_name','bank_account_name','bank_account_number'])data[k]=String(data[k]||'').trim()||null;
 if(!savedCustomerId){
  const x=await window.sb.rpc('staff_create_customer_auto',{p_name:f.get('name'),p_pin:f.get('pin'),p_phone:f.get('phone'),p_id_number:f.get('idn'),p_address:f.get('address'),p_work_salary:[f.get('occupation'),f.get('employer'),f.get('monthly_salary')].filter(Boolean).join(' | '),p_emergency:[f.get('emergency_name'),f.get('emergency_relation'),f.get('emergency_phone')].filter(Boolean).join(' | '),p_internal_notes:f.get('notes')});
  if(x.error)throw x.error;
  const created=Array.isArray(x.data)?x.data[0]:x.data;
  savedCustomerId=created?.id;
  if(!savedCustomerId){submit.dataset.unknownCreation='true';throw Error(L('创建结果未返回客户 ID。请关闭窗口并检查客户列表后再操作。','Creation returned no customer ID. Close this form and check the customer list before proceeding.','Tiada ID pelanggan dikembalikan. Semak senarai pelanggan dahulu.'));}
 }
 const customerId=savedCustomerId;
 const x=await window.sb.from('customers').update(data).eq('id',customerId).select('*').single();
 if(x.error)throw x.error;
 if(!x.data)throw Error('Customer update returned no row.');
 for(const k of ['bank_name','bank_account_name','bank_account_number'])if(String(x.data[k]||'').trim()!==String(data[k]||''))throw Error(L('银行资料保存验证失败，请检查数据库设置。','Bank details could not be verified. Check the database setup.','Maklumat bank tidak dapat disahkan.'));
 for(const [k] of fileTypes){const file=f.get('doc_'+k);if(file&&file.size&&!uploaded.has(file)){await uploadDoc(customerId,k,file);uploaded.add(file)}}
 window.closeModal();window.toast(L('已保存','Saved','Disimpan'));await window.loadAll?.();window.openCustomerProfile?.(customerId);
 }catch(err){window.toast(L('资料未完整保存。','Details were not fully saved. ','Maklumat belum disimpan sepenuhnya. ')+(savedCustomerId?L('客户已建立，可在本窗口重试保存。','Customer exists; retry saving in this form. ','Pelanggan telah wujud; cuba simpan semula. '):'')+(err.message||String(err)),true)}finally{saving=false;submit.disabled=submit.dataset.unknownCreation==='true'}
